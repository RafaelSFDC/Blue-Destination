...
export default async function TestimonialsPage() {
  ...
  const testimonialsByDestination = destinations
    .map((destination) => {
      const packageIds = packages.filter((pkg) => pkg.destinations.includes(destination.id)).map((pkg) => pkg.id)

      const destinationTestimonials = testimonials.filter((testimonial) => packageIds.includes(testimonial.packageId))
  ...
  const testimonialsByPackage = packages
    .map((pkg) => {
      const packageTestimonials = testimonials.filter((testimonial) => testimonial.packageId === pkg.id)
      ...
      <main className="flex-1">
        ...
        <section className="bg-primary py-16 text-white">
          <div className="container text-center">
            <h1 className="mb-4 text-4xl font-bold">Avaliações e Depoimentos</h1>
            <p className="mx-auto max-w-2xl text-lg">
              Descubra o que nossos clientes dizem sobre suas experiências com a Blue Destination.
            </p>
          </div>
        </section>
        ...
        <section className="container py-12">
          <div className="grid gap-8 md:grid-cols-2">
            <div>
              <h2 className="mb-6 text-2xl font-bold">O que nossos clientes acham</h2>
              <div className="mb-8 flex items-center gap-4">
                <div className="flex flex-col items-center justify-center rounded-lg bg-primary/10 p-6 text-center">
                  ...
                  <StarRating rating={averageRating} size={24} className="my-2" />
                  <span className="text-sm text-muted-foreground">Baseado em {testimonials.length} avaliações</span>
                </div>
              ...
              <div className="rounded-lg border p-6">
                <h3 className="mb-4 text-lg font-medium">Compartilhe sua experiência</h3>
                <p className="mb-4 text-muted-foreground">
                  Viajou conosco recentemente? Adoraríamos ouvir sobre sua experiência. Compartilhe sua avaliação e
                  ajude outros viajantes a planejar suas próximas aventuras.
                </p>
                <Button asChild>
          ...
          <div className="container">
            <Tabs defaultValue="all" className="w-full">
                  ...
                  {testimonials.map((testimonial) => (
                    <Card key={testimonial.id}>
                      <CardContent className="p-6">
                        ...
                        <p className="mb-3 text-muted-foreground">{testimonial.comment}</p>

                        <div className="mt-4 text-sm">
                          <span className="font-medium">Pacote: </span>
                          <Link href={`/packages/${testimonial.packageId}`} className="text-primary hover:underline">
                            {packages.find((p) => p.id === testimonial.packageId)?.name}
                          </Link>
                        </div>
                      </CardContent>
                    </Card>
              ...
              <TabsContent value="destinations" className="space-y-8">
                {testimonialsByDestination.map(({ destination, testimonials }) => (
                  <div key={destination.id} className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xl font-bold">{destination.name}</h3>
                      <Button variant="outline" size="sm" asChild>
                        <Link href={`/destinations/${destination.id}`}>Ver Destino</Link>
                      </Button>
                    </div>
                    ...
                    {testimonials.length > 3 && (
                      <div className="text-center">
                        <Button variant="outline" asChild>
                          <Link href={`/destinations/${destination.id}#reviews`}>
                ...
                {testimonialsByPackage.map(({ package: pkg, testimonials }) => (
                  <div key={pkg.id} className="space-y-4">
                    ...
                    {testimonials.length > 3 && (
                      <div className="text-center">
                        <Button variant="outline" asChild>
                          <Link href={`/packages/${pkg.id}#reviews`}>Ver mais avaliações sobre {pkg.name}</Link>
                        </Button>
                      </div>
                    )}
                  </div>
        ...
        <section className="container py-16">
          <div className="rounded-lg border bg-white p-8 text-center shadow-sm">
            <h2 className="mb-4 text-2xl font-bold">Pronto para criar suas próprias memórias?</h2>
            <p className="mx-auto mb-6 max-w-2xl text-muted-foreground">
              Junte-se aos milhares de clientes satisfeitos e comece a planejar sua próxima aventura com a Blue
              Destination.
            </p>
