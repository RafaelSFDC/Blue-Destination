    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Gerencie os destinos disponíveis no site.</p>
        </div>
        <Button asChild>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Total de {destinations.length} destino{destinations.length !== 1 && "s"} cadastrado
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <div className="relative h-10 w-16 overflow-hidden rounded">
                          <Image
                            src={destination.imageUrl || "/placeholder.svg?height=40&width=64"}
                        ...
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">ID: {destination.id}</div>
                        </div>
                      </div>
                    </TableCell>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Destinos marcados como destaque no site</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold">{destinations.filter((dest) => dest.featured).length}</div>
            <p className="text-xs text-muted-foreground">
              {Math.round((destinations.filter((dest) => dest.featured).length / destinations.length) * 100)}% do total
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              <div className="text-3xl font-bold">
                {(destinations.reduce((sum, dest) => sum + dest.rating, 0) / destinations.length).toFixed(1)}
              </div>
              <Star className="h-6 w-6 text-yellow-400 fill-yellow-400" />
            </div>
            <p className="text-xs text-muted-foreground">
              De {destinations.reduce((sum, dest) => sum + dest.reviewCount, 0)} avaliações
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">
              {formatCurrency(destinations.reduce((sum, dest) => sum + dest.price, 0) / destinations.length)}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">
              {new Set(destinations.map((dest) => dest.region).filter(Boolean)).size}
            </div>
