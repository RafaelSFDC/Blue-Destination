...
import { loadCollectionIds, createDocuments, documentExists, generateISODate, generateFutureTravelDate, faker, getRandomDocuments } from './utils';
...
const PASSENGERS_COLLECTION_ID = collectionIds['NEXT_PUBLIC_COLLECTION_PASSENGERS'];
...
async function createPassengers(numPassengers: number) {
  ...
  // Cria os passageiros
  const passengerIds = await createDocuments(PASSENGERS_COLLECTION_ID, passengersData);
...
async function createPayment(bookingId: string, amount: number, status: string) {
  ...
  // Cria o pagamento
  try {
    const paymentId = await createDocuments(PAYMENTS_COLLECTION_ID, [paymentData]);
    return paymentId[0];
  } catch (error) {
...
async function seedBookings() {
  ...
  const bookingExists = await documentExists(BOOKINGS_COLLECTION_ID, 'status', 'confirmed');
  if (bookingExists) {
  ...
  for (let i = 0; i < NUM_BOOKINGS; i++) {
    ...
    // Cria a reserva
    try {
      const bookingIds = await createDocuments(BOOKINGS_COLLECTION_ID, [bookingData]);
      const bookingId = bookingIds[0];
      
      // Cria o pagamento associado à reserva
      const paymentId = await createPayment(bookingId, totalPrice, paymentStatus);
